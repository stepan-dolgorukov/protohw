...
if __name__ == '__main__':
    ...
    if args.aid is None:
        print("Не задан идентификатор альбома.", file=sys.stderr)
        exit(1)

    if args.dir is None:
        print("Не задан каталог с файлами на загрузку.", file=sys.stderr)
        exit(1)
    ...
    for number_file, file in enumerate(
        ...
        try:
            ...
            print(f"Не удалось загрузить файл {file}", file=sys.stderr)
            print(err, file=sys.stderr)
            continue
